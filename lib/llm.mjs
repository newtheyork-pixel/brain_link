...
//
// To run fully offline (nothing leaves the machine):
//   ~/.local/ollama/ollama serve
//   STILLME_LLM_HOST=http://localhost:11434 STILLME_LLM_MODEL=qwen3:4b node server.mjs

const REMOTE = { host: 'https://llm.thegriffinfund.org', model: 'qwen3:14b' };
...
async function backend() {
  ...
  if (process.env.STILLME_LLM_HOST || process.env.STILLME_LLM_MODEL) {
    picked = {
      host: process.env.STILLME_LLM_HOST ?? REMOTE.host,
      model: process.env.STILLME_LLM_MODEL ?? REMOTE.model,
      where: 'override',
    };
    return picked;
  }
  if (await reachable(REMOTE)) picked = { ...REMOTE, where: 'remote GPU (best quality)' };
  else if (await reachable(LOCAL)) picked = { ...LOCAL, where: 'on-device fallback' };
  else throw new Error('no model reachable — check the llm.thegriffinfund.org tunnel');
...
const TILES_SCHEMA = {
  type: 'object',
  properties: {
    tiles: { type: 'array', items: { type: 'string', maxLength: 24 }, minItems: 8, maxItems: 8 },
  },
  required: ['tiles'],
};
...
(He is ASKING A QUESTION — nobody prompted him. The tiles must continue a QUESTION:
 "Emma" → ["did she decide", "the job", "is she happy", "did she call", "how is she", "when", "is she coming", "which one"])` : ''}${!partner && mode === 'tell' ? `
...
export async function health() {
  ...
  return { ...b, offline: b.where === 'on-device' };
}
