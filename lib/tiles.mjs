...
// LLMs emit the curly apostrophe (U+2019) constantly, and the old filter DELETED it — so
// "don't" and "don’t" were different tiles and never deduped, and every digit vanished.
...
/**
 * The model echoes what he already picked back into every tile: he picks "Michael" and
 * gets ["Michael come back", "Michael go", "Michael stay"] — so his sentence reads
 * "Michael Michael leave Michael stay home".
 *
 * A tile is the NEW part of the phrase, never a restatement of it. Strip the echo here
 * rather than trusting a prompt never to do it again: "Michael come back" -> "come back".
 */
export function stripEcho(tile, selected = []) {
  let t = String(tile).trim();
  // Longest first, so "Michael Reyes" is removed before "Michael".
  for (const s of [...selected].sort((a, b) => b.length - a.length)) {
    const esc = String(s).trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    if (!esc) continue;
    t = t.replace(new RegExp(`^\\s*${esc}\\b[\\s,]*`, 'i'), '').trim();
  }
  return t;
}
 ...
 * research claim.
 */
export function buildGrid({ core = [], predicted = [], selected = [], coreSlots = 2, size = 8 }) {
  const pinned = core.slice(0, coreSlots);
  const cleaned = predicted
    .map((t) => stripEcho(t, selected))   // "Michael come back" -> "come back"
    .filter(Boolean);
  const rest = dedupe(cleaned, [...pinned, ...selected]).slice(0, size - pinned.length);
  return [...pinned, ...rest];
}
